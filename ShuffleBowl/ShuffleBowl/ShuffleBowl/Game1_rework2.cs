...
namespace ShuffleBowl
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (gameStarted)
            {
                                    ...
                                    || (isPressed == TouchLocationState.Moved))
                            {
                                ...
                                if ((isPressed == TouchLocationState.Released && gameState == GAMEMODE.RUNNING_SWIPING))
                                {
                                    if (curPos != lastPos)
                                    {
                                        ...
                                        gameState = GAMEMODE.RUNNING_INMOTION;
                                        playerBody.ApplyLinearImpulse(new Vector2(20, -20));
                                    }
                                }
                ...
                else //in motion
                {
                    if (isStopped())
                    {
                        ...
                        {
                            case (4):
                                if (checkIfHighScore(hs, collisionCount))
                    ...
                    else
                    {
                        ...
                        foreach (TouchLocation tl in touchCollection)
                        {
                            if ((tl.State == TouchLocationState.Pressed || tl.State == TouchLocationState.Moved) && stillSwiping == false)
                                speedFactor = 5;
                ...
                if (gameState == GAMEMODE.NAMECOMPLETE)
                {
                    using (IsolatedStorageFile savegameStorage = IsolatedStorageFile.GetUserStoreForApplication())
                    {
                        ...
                        using (fs = savegameStorage.CreateFile(HighScoresFilename))
                        {
                            if (fs != null)
                            {
                                SaveHighScores(hs, fs, collisionCount);
                            }
                        }
                    }
                    gameState = GAMEMODE.MENU;
            ...
            if (gameStarted)
            {
                world.Step(gameTime.ElapsedGameTime.Milliseconds * speedFactor);
            }
            //TimeSpan.FromTicks(333333 * speedFactor);
